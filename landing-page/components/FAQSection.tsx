          ...
          {faqs.map((faq, idx) => {
            ...
            return (
              ...
              >
                <button
                  onClick={() => toggle(idx)}
                  className="w-full flex justify-between items-center text-left py-3 gap-4 group focus:outline-none cursor-pointer"
                >
                  <span className={`font-serif italic text-xl md:text-2xl transition-colors duration-300 ${isOpen ? "text-[#B89768]" : "text-[#4A3B32] group-hover:text-[#B89768]"}`}>
                    {faq.question}
                  </span>
                  ...
                  {isOpen && (
                    ...
                    >
                      <div className="pt-6 pb-2 max-w-3xl">
                        ...
                        {/* Risposta Diretta / Lead in evidenza */}
                        <div className="mb-5 pb-3 border-b border-[#B89768]/20">
                          <p className="font-serif italic text-lg md:text-xl text-[#4A3B32] leading-snug">
                            {faq.lead}
                          </p>
                        </div>

                        {/* Corpo dei paragrafi con spaziatura ariosa */}
                        <div className="space-y-4 text-base md:text-[1.05rem] font-sans font-light leading-[1.8] text-[#4A3B32]/85">
                          {faq.paragraphs.map((paragraph, pIdx) => (
                            <p key={pIdx}>{paragraph}</p>
                          ))}
                        </div>

                        {/* Box Citazione / Takeaway Finale con Bordo Oro */}
                        {faq.quote && (
                          <div className="border-l-2 border-[#B89768] pl-5 py-3.5 my-6 bg-[#F5EFE6]/60 rounded-r">
                            <p className="font-serif italic text-base md:text-lg text-[#4A3B32] leading-relaxed">
